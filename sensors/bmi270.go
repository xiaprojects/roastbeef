// Package sensors provides a stratux interface to sensors used for AHRS calculations.
package sensors

import (
	"encoding/binary"
	"fmt"
	"log"
	"time"

	"github.com/kidoman/embd"
)

// --- BMI270 (BMI2) register map (datasheet + Bosch BMI2 defs) ---
const (
	BMI270_I2C_PRIM_ADDR = 0x68
	BMI270_I2C_SEC_ADDR  = 0x69

	BMI270_CHIP_ID_ADDR   = 0x00
	BMI270_CHIP_ID_VAL    = 0x24 // BMI270 chip-id (datasheet/Bosch defs)
	BMI270_STATUS_ADDR    = 0x03
	BMI270_INTERNAL_STAT  = 0x21
	BMI270_ACC_X_LSB_ADDR = 0x0C
	BMI270_GYR_X_LSB_ADDR = 0x12

	BMI270_INIT_CTRL_ADDR  = 0x59
	BMI270_INIT_ADDR_0     = 0x5B
	BMI270_INIT_ADDR_1     = 0x5C
	BMI270_INIT_DATA_ADDR  = 0x5E
	BMI270_PWR_CONF_ADDR   = 0x7C
	BMI270_PWR_CTRL_ADDR   = 0x7D
	BMI270_CMD_REG_ADDR    = 0x7E
	BMI270_SOFT_RESET_CMD  = 0xB6
	apsDisableDelay        = 1 * time.Millisecond // datasheet says >=450us
	internalStatusTimeout  = 200 * time.Millisecond
	internalStatusPollStep = 5 * time.Millisecond
)

// PWR_CTRL bits (Bosch defs)
const (
	pwrCtrlAuxEn  = 0x01
	pwrCtrlGyrEn  = 0x02
	pwrCtrlAccEn  = 0x04
	pwrCtrlTempEn = 0x08
)

// Example “normal mode” configs from datasheet quick start (you can tune later). [page:0]
const (
	// 200 Hz gyro, filter_perf=1 (aliasing free) + noise_perf=0 => 0xA9 from datasheet example
	BMI270_GYR_CONF_ADDR  = 0x42
	BMI270_GYR_CONF_200HZ = 0xA9

	// 100 Hz accel, filter_perf=1 (aliasing free) => 0xA8 from datasheet example
	BMI270_ACC_CONF_ADDR  = 0x40
	BMI270_ACC_CONF_100HZ = 0xA8

	// Range registers exist in BMI270; keep your previous choices but with correct addresses if needed.
	// If your board needs explicit range set, add the correct registers from datasheet section 5.
)

const (
	BMI270_ACC_RANGE_ADDR = 0x41
	BMI270_GYR_RANGE_ADDR = 0x43

	// Datasheet examples:
	BMI270_ACC_CONF_MAX_FILTER = 0xA8 // acc_filter_perf=1, acc_bwp=normal, acc_odr=100Hz [web:10]
	BMI270_GYR_CONF_MAX_NOISE  = 0xE9 // gyr_filter_perf=1 + gyr_noise_perf=1, gyr_bwp=normal, gyr_odr=200Hz [web:10]

	// Range encodings are defined by Bosch BMI2 API (bmi2_defs.h) for BMI270. [web:4]
	BMI2_ACC_RANGE_8G  = 0x02
	BMI2_GYR_RANGE_125 = 0x04
)

const (
	BMI270_TEMP_0_ADDR = 0x22 // TEMPERATURE_0 (LSB) [web:59]
	BMI270_TEMP_1_ADDR = 0x23 // TEMPERATURE_1 (MSB) [web:59]
)

// --- REQUIRED: Bosch BMI270 config blob ---
// You must paste the byte array from Bosch Sensor-API (bmi270_config_file).
// The datasheet points to the exact file in Bosch repository. [page:0]
var bmi270ConfigFile = []byte{
	}

type BMI270 struct {
	Bus  *embd.I2CBus
	Addr byte

	lastRead time.Time

	accelScale float64 // m/s² per LSB
	gyroScale  float64 // °/s per LSB

	lastGyroX, lastGyroY, lastGyroZ    float64
	lastAccelX, lastAccelY, lastAccelZ float64
	lastTemp                           float64
}

func NewBMI270(i2cbus *embd.I2CBus) (*BMI270, error) {
	m := &BMI270{
		Bus:      i2cbus,
		Addr:     BMI270_I2C_PRIM_ADDR,
		lastRead: time.Now(),
	}

	// 1) Soft reset (CMD register), then wait for registers accessible. [page:0]
	if err := m.writeReg(BMI270_CMD_REG_ADDR, BMI270_SOFT_RESET_CMD); err != nil {
		return nil, fmt.Errorf("soft reset failed: %w", err)
	}
	time.Sleep(10 * time.Millisecond)

	// 2) Read chip-id (sanity check).
	chip, err := m.readReg(BMI270_CHIP_ID_ADDR)
	if err != nil {
		return nil, fmt.Errorf("chip-id read failed: %w", err)
	}
	if chip != BMI270_CHIP_ID_VAL {
		// Some modules can present different IDs (variant/derivative), but 0 usually means bus issue.
		log.Printf("BMI270: unexpected chip-id 0x%02X (expected 0x%02X)\n", chip, BMI270_CHIP_ID_VAL)
	}

	// 3) Mandatory init: disable adv power save, upload config file, wait internal status. [page:0]
	if err := m.loadConfig(bmi270ConfigFile); err != nil {
		return nil, err
	}

	// 4) Enable acc + gyro + temp in normal mode example. [page:0]
	// Enable sensors
	if err := m.writeReg(BMI270_PWR_CTRL_ADDR, pwrCtrlAccEn|pwrCtrlGyrEn|pwrCtrlTempEn); err != nil {
		return nil, fmt.Errorf("pwr ctrl failed: %w", err)
	}
	time.Sleep(2 * time.Millisecond)

	// Set ranges
	_ = m.writeReg(BMI270_ACC_RANGE_ADDR, BMI2_ACC_RANGE_8G)
	_ = m.writeReg(BMI270_GYR_RANGE_ADDR, BMI2_GYR_RANGE_125)

	// Max noise filtering / best quality settings (datasheet examples)
	_ = m.writeReg(BMI270_GYR_CONF_ADDR, BMI270_GYR_CONF_MAX_NOISE)
	_ = m.writeReg(BMI270_ACC_CONF_ADDR, BMI270_ACC_CONF_MAX_FILTER)

	time.Sleep(20 * time.Millisecond)

	// datasheet “normal mode” example values (optional but good baseline). [page:0]
	_ = m.writeReg(BMI270_GYR_CONF_ADDR, BMI270_GYR_CONF_200HZ)
	_ = m.writeReg(BMI270_ACC_CONF_ADDR, BMI270_ACC_CONF_100HZ)

	// 5) Scaling factors (16-bit signed).
	// accel: for ±4g -> 8192 LSB/g (datasheet sensitivity table). [page:0]
	// gyro: for ±250 dps -> 131.072 LSB/dps (datasheet sensitivity table). [page:0]
	// If you change range, update these.
	m.accelScale = 1.0 / 4096.0 // m/s² per LSB @ ±8g [web:10]
	m.gyroScale = 1.0 / 262.144 // dps per LSB @ ±125 dps [web:10]

	time.Sleep(50 * time.Millisecond)
	return m, nil
}

func (m *BMI270) loadConfig(cfg []byte) error {
	// BMI270 config is 8192 bytes (256 * 32). If you have a different length, fail early.
	if len(cfg) != 8192 {
		return fmt.Errorf("bmi270 config blob must be 8192 bytes, got %d", len(cfg))
	}

	// Disable advanced power save: PWR_CONF=0x00, wait >=450us. [web:10]
	if err := m.writeReg(BMI270_PWR_CONF_ADDR, 0x00); err != nil {
		return fmt.Errorf("disable adv power save failed: %w", err)
	}
	time.Sleep(apsDisableDelay)

	// Prepare config load: INIT_CTRL=0x00. [web:10]
	if err := m.writeReg(BMI270_INIT_CTRL_ADDR, 0x00); err != nil {
		return fmt.Errorf("init ctrl prepare failed: %w", err)
	}
	time.Sleep(1 * time.Millisecond)

	// Write config in 256 blocks of 32 bytes. [web:10]
	for block := 0; block < 256; block++ {
		// INIT_ADDR uses two registers; commonly INIT_ADDR_0=0x00 and INIT_ADDR_1=block index. [web:20]
		if err := m.writeReg(BMI270_INIT_ADDR_0, 0x00); err != nil {
			return fmt.Errorf("init addr0 write failed: %w", err)
		}
		if err := m.writeReg(BMI270_INIT_ADDR_1, byte(block)); err != nil {
			return fmt.Errorf("init addr1 write failed: %w", err)
		}

		start := block * 32
		chunk := cfg[start : start+32]

		// Burst write 32 bytes to INIT_DATA. [web:10]
		if err := (*m.Bus).WriteToReg(uint8(m.Addr), uint8(BMI270_INIT_DATA_ADDR), chunk); err != nil {
			return fmt.Errorf("init data write failed at block=%d: %w", block, err)
		}
	}

	// Complete config load: INIT_CTRL=0x01. [web:10]
	if err := m.writeReg(BMI270_INIT_CTRL_ADDR, 0x01); err != nil {
		return fmt.Errorf("init ctrl complete failed: %w", err)
	}

	// Wait up to 20ms; datasheet says INTERNAL_STATUS should become 0x01 when OK. [web:10]
	deadline := time.Now().Add(50 * time.Millisecond)
	for time.Now().Before(deadline) {
		st, err := m.readReg(BMI270_INTERNAL_STAT)
		if err == nil && (st&0x01) == 0x01 {
			return nil
		}
		if err == nil && (st&0x02) == 0x02 {
			// init_err latched -> advise power cycle (seen in the field). [web:20]
			return fmt.Errorf("bmi270 init_err latched (INTERNAL_STATUS=0x%02X): power-cycle sensor and retry", st)
		}
		time.Sleep(internalStatusPollStep)
	}

	st, _ := m.readReg(BMI270_INTERNAL_STAT)
	return fmt.Errorf("bmi270 init timeout (INTERNAL_STATUS=0x%02X)", st)
}

func (m *BMI270) readReg(reg byte) (byte, error) {
	buf := []byte{0}
	if err := (*m.Bus).ReadFromReg(uint8(m.Addr), uint8(reg), buf); err != nil {
		return 0, err
	}
	return buf[0], nil
}

func (m *BMI270) writeReg(reg, value byte) error {
	return (*m.Bus).WriteToReg(uint8(m.Addr), uint8(reg), []byte{value})
}

func (m *BMI270) readAccel() (ax, ay, az float64, err error) {
	buf := make([]byte, 6)
	if err := (*m.Bus).ReadFromReg(uint8(m.Addr), BMI270_ACC_X_LSB_ADDR, buf); err != nil {
		return 0, 0, 0, err
	}

	// BMI270 data is little-endian: LSB first. [page:0]
	x := int16(binary.LittleEndian.Uint16(buf[0:2]))
	y := int16(binary.LittleEndian.Uint16(buf[2:4]))
	z := int16(binary.LittleEndian.Uint16(buf[4:6]))

	return float64(x) * m.accelScale, float64(y) * m.accelScale, float64(z) * m.accelScale, nil
}

func (m *BMI270) readGyro() (gx, gy, gz float64, err error) {
	buf := make([]byte, 6)
	if err := (*m.Bus).ReadFromReg(uint8(m.Addr), BMI270_GYR_X_LSB_ADDR, buf); err != nil {
		return 0, 0, 0, err
	}

	// little-endian. [page:0]
	x := int16(binary.LittleEndian.Uint16(buf[0:2]))
	y := int16(binary.LittleEndian.Uint16(buf[2:4]))
	z := int16(binary.LittleEndian.Uint16(buf[4:6]))

	return float64(x) * m.gyroScale, float64(y) * m.gyroScale, float64(z) * m.gyroScale, nil
}

func (m *BMI270) readTempC() (float64, error) {
	// Temperature is 16-bit, 512 LSB/K, 0 LSB at 23°C. [page:0]
	buf := make([]byte, 2)
	if err := (*m.Bus).ReadFromReg(uint8(m.Addr), BMI270_TEMP_0_ADDR, buf); err != nil {
		return 0, err
	}
	raw := int16(binary.LittleEndian.Uint16(buf))
	return 23.0 + float64(raw)/512.0, nil
}

// Read returns average time since last read (ns), gyro (dps), accel (m/s²), mag unused.
func (m *BMI270) Read() (T int64, G1, G2, G3, A1, A2, A3, M1, M2, M3 float64, GAError, MagError error) {
	now := time.Now()
	T = now.Sub(m.lastRead).Nanoseconds()

	gx, gy, gz, err := m.readGyro()
	if err != nil {
		return T, 0, 0, 0, 0, 0, 0, 0, 0, 0, err, nil
	}
	ax, ay, az, err := m.readAccel()
	if err != nil {
		return T, 0, 0, 0, 0, 0, 0, 0, 0, 0, err, nil
	}
	tmp, _ := m.readTempC()

	m.lastGyroX, m.lastGyroY, m.lastGyroZ = gx, gy, gz
	m.lastAccelX, m.lastAccelY, m.lastAccelZ = ax, ay, az
	m.lastTemp = tmp
	m.lastRead = now

	return T, gx, gy, gz, ax, ay, az, 0, 0, 0, nil, nil
}

// ReadOne returns current timestamp (ns) and current gyro/accel.
func (m *BMI270) ReadOne() (T int64, G1, G2, G3, A1, A2, A3, M1, M2, M3 float64, GAError, MagError error) {
	T = time.Now().UnixNano()

	gx, gy, gz, err := m.readGyro()
	if err != nil {
		return T, 0, 0, 0, 0, 0, 0, 0, 0, 0, err, nil
	}
	ax, ay, az, err := m.readAccel()
	if err != nil {
		return T, 0, 0, 0, 0, 0, 0, 0, 0, 0, err, nil
	}

	return T, gx, gy, gz, ax, ay, az, 0, 0, 0, nil, nil
}

// Close: bus is managed externally.
func (m *BMI270) Close() {}
