...
function GPSCtrl($rootScope, $scope, $state, $http, $interval) {
    ...
    // G-Load Buzzer, by default it is disabled because to play audio there is the need of user interaction
    $scope.gMeterBuzzerVolume = window.gMeterBuzzerPlayer.volume;
    $scope.setGMeterBuzzVolume = function(volume100) {
        window.gMeterBuzzerPlayer.setVolumeTo(volume100);
...
function AudioSinusoidPlayer() {
    ...
    this.synthByFreq = {}; // Cache of the wave buffer
    this.volume = 0; // by default it is disabled
    this.seconds = 0.20; // Buzz duration
    this.waitFor = 300; // Buzz interval
    this.quietRange = 0.3; // G: 0.7<-G->1.3 will be quiet
    this.max = 4.4; // Default G, used to calculate the frequency
    this.min = -1.74; // Default G
    this.maxHz = 1320; // Max Hz frequency mapped to max G
    this.minHz = 220; // Same for Min
    this.lastPlayTime = new Date(); // Last Play date to avoid overlapping
